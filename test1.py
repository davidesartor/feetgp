...
# Variants
############################################################
warm_start_theta = True
warm_start_g = True
reverse_warm_start_order = True  # True -> start with high lambda
log_scale_plots = True
...
fig.write_html("test1/function_surface.html")
print("Done!")
print()
...
if reverse_warm_start_order:
    lambdas = lambdas[::-1]
for l in lambdas:
    previous_theta = (
        models[-1].parameters.theta if models and warm_start_theta else None
    )
    previous_g = models[-1].parameters.g if models and warm_start_g else None
    model = gp.GaussianProcessRegressor(
        l1_penalty=l,
        max_admm_iterations=1000,
        max_bfgs_iterations=10,
        tollerance=1e-4,
        verbose=True,
        init_theta=previous_theta,
        init_g=previous_g,
    )
    ...
    np.savez(
        f"test1/model_{len(models)}_l1={l:.1e}.npz",
        l1_penalty=l,
    ...
    fig.write_html(f"test1/optimization_trajectory_{len(models)}_l1={l:.8f}.html")
    print("Done!")
    print()
...
fig.write_html(
    f"test1/effect_of_regularization_when_warm_start={warm_start_g or warm_start_theta}.html"
)
print("Done!")
print()
