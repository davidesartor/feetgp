...
Nothing else in the JAX ecosystem ships a bounded L-BFGS — `optimistix` and `optax` have LBFGS only, `jaxopt` has `LBFGSB` but is deprecated — so this repo carried its own Byrd–Lu–Nocedal–Zhu port (`lbfgsb.py`) plus a scipy-fidelity battery (`tests/test_lbfgsb.py`). Both were deleted on 2026-08-04: the solver ships in `vlse` (`jaxvlse` on PyPI, imports as `vlse`), and its correctness tests go with it. `glassogp.py` was never wired to it and still calls `optx.LBFGS` with a projection.

`bench/bench_lbfgsb.py` stays, and imports `from vlse.lbfgsb import minimise` — a placeholder path, since the upstream release is still in flight. `vlse` is about ready to ship **0.1.0**; when it lands, `uv add jaxvlse` and the import resolves as written. Until then anything importing `vlse` is broken (`ModuleNotFoundError: No module named 'vlse.lbfgsb'`), which is the one thing gating the L-BFGS-B x-update merge (`docs/HANDOFF.md`, "The planned integration", step 2).
