...
for l in lambdas:
    ...
    np.savez(
        f"test4/model_{len(models)}_l1={l:.1e}.npz",
        l1_penalty=l,
...
r2s = jnp.array(
    [
        [r2_score(y_test[:, i], y_pred.mean[i, :]) for y_pred in y_preds]
        for i in range(o)
    ]
)
thetas = jnp.array([model.parameters.theta for model in models])
gs = jnp.array([model.parameters.g.sum() for model in models])

r2s = rearrange(r2s, "(o k) i -> i o k", k=3).mean(-1)  # average over same group
thetas = rearrange(thetas, "i o (d k) -> i (o k) d", k=3).sum(-2)  # sum over same group

fig = make_subplots(rows=2, cols=1)
for i in range(markers):
    fig.add_trace(
        go.Scatter(x=lambdas, y=r2s[:, i], mode="markers+lines", name=f"R2_{i}"),
        row=1,
        col=1,
    )
...
fig.write_html(f"test4/effect_of_regularization.html")
print("Done!")
print()
